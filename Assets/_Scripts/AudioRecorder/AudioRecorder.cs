...
namespace Recorder
{
    ...
    public class AudioRecorder : MonoBehaviour
    {
        ...
        #region Private Vars
        private Character.Type _current_character_type;
        #endregion
        ...
        private void Start()
        {
            UpdateRecordWindow(Character.Type.Black);
        }

        public void UpdateRecordWindow(Character.Type character_type)
        {
            ...
            audioSource.clip = LoadAudioFromFile(Path.Combine(Application.persistentDataPath, _current_character_type.ToString() + ".wav"));
        ...
        {
            fileName = _current_character_type.ToString();
        ...
        public AudioClip LoadAudioFromFile(string filePath)
        {
            if (!File.Exists(filePath))
            {
                Debug.LogError($"Файл по пути {filePath} не найден!");
                return null;
            }
            ...
            // Преобразуем WAV в AudioClip
            AudioClip clip = WavUtility.ToAudioClip(fileData, Path.GetFileNameWithoutExtension(filePath));
...
{
    public static AudioClip ToAudioClip(byte[] wavFileData, string clipName = "wavClip")
    {
        int channels, frequency;
        float[] audioData = GetWavData(wavFileData, out channels, out frequency);

        if (audioData == null)
            return null;

        AudioClip audioClip = AudioClip.Create(clipName, audioData.Length, channels, frequency, false);
        audioClip.SetData(audioData, 0);
        return audioClip;
    }

    private static float[] GetWavData(byte[] wavFileData, out int channels, out int frequency)
    {
        // Чтение заголовка WAV (44 байта)
        channels = BitConverter.ToInt16(wavFileData, 22);
        frequency = BitConverter.ToInt32(wavFileData, 24);
        int byteRate = BitConverter.ToInt32(wavFileData, 28);

        int dataOffset = 44; // WAV начинается с 44 байт заголовка
        int dataSize = wavFileData.Length - dataOffset;
        int sampleCount = dataSize / 2; // WAV данные представлены в 16-битном формате

        float[] audioData = new float[sampleCount];
        for (int i = 0; i < sampleCount; i++)
        {
            short sample = BitConverter.ToInt16(wavFileData, dataOffset + i * 2);
            audioData[i] = sample / 32768f; // Преобразуем в диапазон [-1.0, 1.0]
        }
