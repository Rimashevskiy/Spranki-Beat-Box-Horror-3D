...
//                  Debug.Log("Multithread time cells (" + threads.Length + " threads) = " + (fEndTime - fStartTime) + " seconds");
        ...
        public static void ComputeChunkColliders(FracturedObject fracturedComponent, ProgressDelegate progress,
            string house = null, int currentItem = 0, int totalItems = 0)
        {
            int nChunk = 0;
            int nTotalFaces = 0;
            ...
            foreach(FracturedChunk fracturedChunk in fracturedComponent.ListFracturedChunks)
            {
                ...
                if(progress != null)
                {
                    progress($"{house} Computing colliders", $"({currentItem}/{totalItems}) Collider {nChunk + 1}/{fracturedComponent.ListFracturedChunks.Count} of \"{fracturedComponent.name}\"", (float)nChunk / (float)fracturedComponent.ListFracturedChunks.Count);
                }
